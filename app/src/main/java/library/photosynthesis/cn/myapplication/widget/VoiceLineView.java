...
/**
 * 自定义声音振动曲线view
 * Created by siqiangli on 2017/5/11 13:02
 */
public class VoiceLineView extends View {
    ...
    private void drawVoiceLine(Canvas canvas) {
        ...
        for (int i = 0; i < paths.size() && isPrepared; i++) {
            paths.get(i).reset();
            paths.get(i).moveTo(getWidth(), getHeight() / 2);
        }
        for (float i = getWidth() - 1; i >= 0 && isPrepared; i -= fineness) {
            amplitude = 4 * volume * i / getWidth() - 4 * volume * i * i / getWidth() / getWidth();
            for (int n = 1; n <= paths.size() && isPrepared; n++) {
        ...
        for (int n = 0; n < paths.size() && isPrepared; n++) {
            if (n == paths.size() - 1) {
